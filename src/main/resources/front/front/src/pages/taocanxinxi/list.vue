...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"1px solid #E0E0E0","padding":"10px","margin":"0 auto","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"calc(100% - 20px)","height":"auto"}'>
      <el-form-item :style='{"margin":"0 5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#000","display":"inline-block"}'>套餐名称：</div>
        <el-input v-model="formSearch.taocanmingcheng" placeholder="套餐名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('taocanxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 10px 0","outline":"none","color":"#fff","borderRadius":"0","background":"#3B2E7E","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/taocanxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"border":"1px solid #E0E0E0","width":"calc(100% - 20px)","margin":"0 auto","borderWidth":"0 1px 1px","justifyContent":"space-around","display":"flex"}'>
		<el-button :style='{"border":"0","padding":"0 15px","margin":"0 5px","borderRadius":"8px"}' @click="sortClick('clicknum')">
			<span :style='{"margin":"0 6px 0 0","lineHeight":"40px","fontSize":"15px","color":"#000"}' class="icon iconfont icon-liulan05" v-if="sortType!='clicknum'"></span>
			<span :style='{"margin":"0 6px 0 0","lineHeight":"40px","fontSize":"15px","color":"#000"}' class="icon iconfont icon-jiantou35" v-else-if="sortType=='clicknum'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 6px 0 0","lineHeight":"40px","fontSize":"15px","color":"#000"}' class="icon iconfont icon-jiantou36" v-else-if="sortType=='clicknum'&&sortOrder=='asc'"></span>
	...
	<div class="list" :style='{"width":"100%","padding":"20px 0","margin":"0 0 10px","background":"#fff"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"block","height":"auto"}'>
			<div :style='{"border":"1px solid #E0E0E0","padding":"45px 30px 15px","margin":"0","flexWrap":"wrap","background":"none","display":"inline-block","width":"25%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.fengmian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px","order":"1"}' v-if="item.fengmian && item.fengmian.substr(0,4)=='http'" :src="item.fengmian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.fengmian?item.fengmian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px","order":"1"}' v-else :src="baseUrl + (item.fengmian?item.fengmian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"10px 0","color":"#444","textAlign":"center","width":"100%","lineHeight":"1.5","fontSize":"20px","fontWeight":"bold","order":"3"}' class="name ">{{item.taocanmingcheng}}</div>
				...
				<div :style='{"width":"calc(100% / 3)","padding":"0 10px","textAlign":"center","display":"inline-block","order":"6"}' v-if="item.clicknum">
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/taocanleixing/taocanleixing').then(res => {
			if (res.data.code == 0) {
				this.taocanleixingOptions = res.data.data;
				this.selectOptionsList.push({name:'套餐类型',list:this.taocanleixingOptions,tableName: 'taocanleixing',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.taocanmingcheng != '') searchWhere.taocanmingcheng = '%' + this.formSearch.taocanmingcheng + '%';
        if (this.formSearch.taocanleixing != '') searchWhere.taocanleixing = this.formSearch.taocanleixing;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`taocanxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/taocanxinxiDetail', query: params});
      },
