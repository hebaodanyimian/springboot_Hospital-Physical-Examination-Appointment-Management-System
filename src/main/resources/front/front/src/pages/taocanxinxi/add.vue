...
<div :style='{"width":"80%","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}'  label="套餐类型" prop="taocanleixing">
            <el-select v-model="ruleForm.taocanleixing" placeholder="请选择套餐类型" :disabled=" false  ||ro.taocanleixing" >
              <el-option
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="可约时间" prop="keyueshijian">
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="套餐详情" prop="taocanxiangqing">
            <editor 
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/taocanleixing/taocanleixing', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`taocanxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('taocanxinxi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`taocanxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
