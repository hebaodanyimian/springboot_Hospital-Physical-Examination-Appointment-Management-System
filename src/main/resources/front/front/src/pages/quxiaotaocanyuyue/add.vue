...
<div :style='{"width":"80%","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="取消编号" prop="quxiaobianhao">
              <el-input v-model="ruleForm.quxiaobianhao" placeholder="取消编号" disabled></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="套餐名称" prop="taocanmingcheng">
            <el-input v-model="ruleForm.taocanmingcheng" 
                placeholder="套餐名称" clearable :disabled=" false  ||ro.taocanmingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="套餐类型" prop="taocanleixing">
            <el-input v-model="ruleForm.taocanleixing" 
                placeholder="套餐类型" clearable :disabled=" false  ||ro.taocanleixing"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="取消时间" prop="quxiaoshijian">
              <el-date-picker
				  :disabled="true  ||ro.quxiaoshijian"
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="取消原因" prop="quxiaoyuanyin">
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`quxiaotaocanyuyue/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('quxiaotaocanyuyue/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`quxiaotaocanyuyue/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
