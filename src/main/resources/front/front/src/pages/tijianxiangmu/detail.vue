...
<div>
	<div :style='{"border":"1px solid #E0E0E0","padding":"20px 0 10px","margin":"10px auto","borderRadius":"0","background":"none","borderWidth":"0 0 2px","display":"flex","width":"80%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/tijianxiangmu"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","padding":"40px ","margin":"0 auto","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"80%","position":"relative"}'>
		<div class="attr" :style='{"padding":"0 20px","margin":"0 0 10px","background":"#fff","display":"flex","width":"35%","position":"relative","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0 10px","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#818181","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>可约时间</div>
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.keyueshijian}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#818181","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>注意事项</div>
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.zhuyishixiang}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#818181","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>点击次数</div>
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.clicknum}}</div>
				</div>
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#fff","borderRadius":"20px","background":"#3B2E7E","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('tijianxiangmu','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #CCCCCC","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#CCCCCC","borderRadius":"20px","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('tijianxiangmu','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
					<!-- crossOptButtonStatusColumns  -->
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#fff","borderRadius":"20px","background":"#6353B4","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('tijianxiangmu','体检预约')" @click="onAcross('tijianyuyue','','','','')" type="warning">体检预约</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
        localStorage.setItem('crossTable',`tijianxiangmu`);
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('tijianxiangmu/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'tijianxiangmu', userid: localStorage.getItem('frontUserid')}}).then(res => {
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/tijianxiangmuAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此体检项目？')
			  .then(_ => {
			    this.$http.post('tijianxiangmu/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
