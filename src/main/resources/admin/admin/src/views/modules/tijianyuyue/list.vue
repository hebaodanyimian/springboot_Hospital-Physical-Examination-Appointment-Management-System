<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","margin":"0 0 10px","borderRadius":"0 0 8px 8px","textAlign":"left","background":"#fff","display":"block","width":"100%","fontSize":"inherit","order":"1"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">体检类型</label>
						<el-input v-model="searchForm.tijianleixing" placeholder="体检类型" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('tijianyuyue','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('tijianyuyue','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('tijianyuyue','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('tijianyuyue','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('tijianyuyue','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('tijianyuyue','取消预约')" @click="quxiaotijianyuyueCrossAddOrUpdateHandler(scope.row,'cross','是','是','[1]','请勿重复操作!')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								取消预约
							</el-button>
							<el-button class="btn8" v-if="isAuth('tijianyuyue','体检报告')" @click="tijianbaogaoCrossAddOrUpdateHandler(scope.row,'cross','是','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								体检报告
							</el-button>
							<el-button class="edit" v-if=" isAuth('tijianyuyue','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('tijianyuyue','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<quxiaotijianyuyue-cross-add-or-update v-if="quxiaotijianyuyueCrossAddOrUpdateFlag" :parent="this" ref="quxiaotijianyuyueCrossaddOrUpdate"></quxiaotijianyuyue-cross-add-or-update>
...
import quxiaotijianyuyueCrossAddOrUpdate from "../quxiaotijianyuyue/add-or-update";
import tijianbaogaoCrossAddOrUpdate from "../tijianbaogao/add-or-update";
	export default {
		data() {
			return {
				...
				quxiaotijianyuyueCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			quxiaotijianyuyueCrossAddOrUpdate,
		...
		methods: {
			...
			quxiaotijianyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.quxiaotijianyuyueCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','tijianyuyue');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.quxiaotijianyuyueCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.quxiaotijianyuyueCrossaddOrUpdate.init(row.id,type);
				});
			},
			tijianbaogaoCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tijianbaogaoCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
			...
			payHandler(row){
				...
				this.$storage.set('paytable','tijianyuyue');
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.tijianleixing!='' && this.searchForm.tijianleixing!=undefined){
            params['tijianleixing'] = '%' + this.searchForm.tijianleixing + '%'
          }
    ...
    shBatchHandler(){
      this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
        ...
        this.$http({
          url: "tijianyuyue/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
